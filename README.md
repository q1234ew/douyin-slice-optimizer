...
- [抖音采集标准](docs/guides/douyin-collection-standard.md)：采集边界、字段规范、质量门和去重规则。
...
- ASR 本地优先：默认 `DSO_ASR_BACKEND=auto`，音乐综艺 `program/precut` 优先使用 Qwen3-ASR，失败时回退 `whisper.cpp`，再回退 faster-whisper。项目会自动识别 `tools/whisper.cpp`、`data/models/whisper.cpp` 和显式 Whisper 路径，保证局域网 GPU 不可用时主流程仍可运行。
- Qwen3-ASR 音乐综艺生产主后端：`input_mode=program/precut` 且未显式指定其他后端时，`dso extract` 优先使用 GPU 服务器上的 `Qwen/Qwen3-ASR-1.7B` + `Qwen/Qwen3-ForcedAligner-0.6B` 写主 transcript；模型未加载、服务不可用或转写失败时自动回退 Whisper，不生成占位结果。`dso qwen3-asr-shadow <video_id>` 仍保留为独立对照入口，不会覆盖主转写。`model_scheduler.v1` 已支持逐块 ASR、逐窗口 Omni、Embedding 和受控 Resource Agent；局域网 canary 已在 `192.168.31.143` 部署 Agent，并通过 launchd 显式启用 Worker/8127 Web，其他环境仍默认关闭。Qwen 默认采用 60 秒、1 秒重叠、5 秒低能量边界搜索和空 context，异常块会受限缩块重试并显式记录质量状态。
...
- ASR 模型路由：音乐综艺默认 `Qwen3-ASR primary -> Whisper fallback`；显式 `--asr-backend whisper_cpp` 可按任务覆盖。`GET /videos/{video_id}/quality` 会给出全片与 Top 候选的路由建议；`dso verify-asr <segment_id> --profile auto` 会按片段信号选择 `quality` 或 `verify`。
...
- Qwen3.5-Omni 公网完整短片研究使用独立 `qwen35_omni_propagation_features` profile：完整 H.264/AAC 音视频只抽取音画事实，平台结果在全部响应后本地连接，模型不输出传播分。`scripts/run_bailian_propagation_features.py` 支持 1-100 条预检/执行、缓存、显式预算和结果缺失语义；当前 10 条 schema pilot 已通过，但仍为 `research_only`，下一门禁是 60 条新配对的账号隔离/时间切分验证。
