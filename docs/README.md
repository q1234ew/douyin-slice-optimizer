...
1. [user-manual.md](user-manual.md): 面向剪辑、运营和管理员的平台用户手册，覆盖安装、Web 工作台、CLI、数据回流和日常 SOP。
2. [current-state.md](current-state.md): 当前工程状态、数据口径、服务启动、验证结果和近期风险。
3. [development-requirements.md](development-requirements.md): 开发约束、测试要求、数据入库规则、前端构建要求。
4. [architecture.md](architecture.md): 系统架构、模块边界、数据模型和 API 草案。
5. [douyin-collection-standard.md](douyin-collection-standard.md): 抖音数据采集标准、合规边界、质量门和去重规则。
6. [douyin-media-collection-flow.md](douyin-media-collection-flow.md): 抖音切片视频媒体采集流程、CLI 命令、目录结构和质量门。
7. [iteration-history-20260629.md](iteration-history-20260629.md): 2026-06-29 历史样本资产、V1 可学习排序闭环的阶段入口。
8. [iteration-history-20260628.md](iteration-history-20260628.md): 2026-06-28 这一轮迭代复盘和后续计划。
...
- 项目是本地优先的音乐综艺切片优化工作台，不自动发布、不刷量、不绕过平台规则。
