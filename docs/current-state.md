...
- 2026-07-20 完成算法阶段 1：以 10,853 条、25 个研究账号冻结 `interaction_heat_labels.v3` 和 `dso-interaction-heat-v3-20260720-r3`。五维结果保留 `like/share/favorite/discussion/broad_heat`，每个目标同时记录实际选中 normalizer 的样本量；两套 normalizer 均只读各自 train 分区。`account_time=train/validation/test 6850/925/3078`，整账号 `train/validation/test=7606/643/1697`，另有 907 条同源冲突显式 `excluded_leakage`；两套 source-group 泄漏均为 0。630/10,853 条有媒体 SHA（5.8048%），其余明确降级到 item/标题/节目键。两轮独立复审后废弃 r1/r2：除 outcome 派生筛选、空值误转零、外部 pinned digest、snapshot alias/发布回滚和 UTC settlement 外，r3 进一步强制 artifact 目录只有 manifest 加固定四 payload、以 `O_NOFOLLOW` 拒绝内外部 symlink 和非普通文件、拒绝 dangling output symlink、按 inode 保护并发回滚、保护自动生成的 git provenance，并以零初值 BudgetGuard 加 UTC day handshake/retry 消除构造期跨午夜污染。正式准入为“至少一项指标有显式 provenance 与值”，有效零仍保留，全缺失的 149 条不进入标签集。ECS r3、下载后的本地 r3 和独立本地复跑四个 payload SHA 逐字节一致；外部 pinned manifest SHA-256 为 `4d4157f145d83be5ef141f3a48f9f7f38d05e13d4d75bc7f730af74fd25a3971`。本阶段网络请求 0、模型费用 0，未覆盖 v2、未改生产排序、Gold、导出或发布。
- 2026-07-20 在本地 Mac 完成算法阶段 2 首个正式 Pairwise Logistic：新增纯 Python、固定 seed 的 4096 维 signed-hash 稀疏模型、train-only 配对、两个协议独立模型、账号宏 NDCG@10/Top-10 heat lift/严重误选报告，以及 `interaction-heat-pairwise-local` CLI。full-feature r1 暴露高基数节目/歌曲/标签过拟合；按 validation 选择的 core 结构 profile 在最终 `dso-interaction-heat-pairwise-20260720-r3` 上，`account_time` test 为 `0.434468`，低于随机 `0.447066`，Top-10 heat lift `-0.004695`；`account_holdout` test 为 `0.424485`，高于随机 `0.327250`，Top-10 heat lift `+0.070676`，但仍明显弱于此前简单结构+标题 target-encoding 探测的 `0.551/0.613`。因此 Pairwise 当前仅是 `research_only` 负向/弱基线，不进入生产候选；r3 manifest SHA-256 为 `834dee35d6ec4dbac5d2351ee94a3ec101258ac55ecfc3f8bec6d9a239292368`。本地训练 2.46 秒，网络请求和模型费用均为 0，数据库、Gold、生产权重、导出与发布均未改变。
- 2026-07-20 完成算法阶段 2 的正式 Target Encoding 基线：`interaction_heat_target_encoding.v1` 使用纯 Python、train-only 层级平滑、按 source-group/账号的 5-fold OOF、显式 missing/fallback 和协议隔离，默认只启用结构、时间/时长与置信度白名单，标题特征关闭；`account_holdout` 不保留账号历史。正式 validation-only artifact 为 `dso-interaction-heat-target-encoding-20260720-r2`，不生成 test 预测或指标，predictions 也不含标签/互动 outcome。账号内时间 validation NDCG@10 为 `0.669155`（随机 `0.583703`）、Top-10 heat lift `+0.053093`；整账号 validation 为 `0.304649`（随机 `0.242408`）、lift `+0.048263`，但仅 3 个账号且严重误选率 `33.3333%`，并弱于 Pairwise 的整账号 validation `0.4238`。整账号 train OOF 还略低于随机（`0.437350` 对 `0.448042`），说明跨账号泛化仍不稳定。修正 test split 提前过滤后，r1/r2 模型和预测逐字节一致；r1 标记 superseded，r2 manifest SHA-256 为 `a118736dab9261e1db0fa11c5c59037f25e5c0ef1091c2b4fd12174e9d3ff98d`，独立重跑三个确定性 payload 一致。该基线保持 `research_only`，不因账号内 validation 正向结果进入生产；本地运行约 2.75 秒，网络请求、模型/API 费用和数据库/Gold/生产权重/导出/发布变更均为 0。
- 2026-07-20 新增 `interaction_heat_holdout_readiness.v1` 与 `interaction-heat-holdout-readiness` 只读门禁。它只从已验签 r3 的 `splits.jsonl` 读取冻结 ID、账号和发布时间 cutoff，不解析标签 target；当前数据库候选继续复用 V3 的显式 metric provenance 准入。默认解锁条件为：前向窗口至少 1,000 条、5 个账号、跨 7 天，并且至少 3 个冻结集外账号各有 100 条。真实本地探针以 pinned r3 manifest 和数据库 SHA `68f55606efd05ed49711b7c55a4456df679da16314c79106ef8373b6ed28e237` 返回 `not_ready`：10,853 条 provenance 合格行全部已冻结，晚于 `2026-06-30T13:38:00+00:00` 的新候选为 0，新账号为 0，`unlock_nonlinear_ranker=false`。因此本轮没有安装 LightGBM、没有运行 LambdaRank，也没有把旧 test 重新包装成新 holdout；网络、模型费用、数据库/Gold/生产 ranker 变更均为 0。
- 后端当前 `pytest -q` 为 `358 passed, 4 warnings`（2026-07-21）；全源码 `compileall`、`git diff --check`、新增行敏感模式扫描和前端 `npm run build` 均通过；本轮另复验 `vue-tsc --noEmit` 通过。UTC 跨午夜结算已修复：旧日 reservation 的实际费用不再污染新日 batch/day 内存额度，runtime 以零初值开始并通过 UTC day handshake/retry 加载当前日 batch/day spend，后续持久刷新也只读取当前 UTC 日，持久 reservation 仍保留原 `budget_day`。真实浏览器最近一次回归仍为桌面/窄屏无横向溢出、控制台 warning/error 为 0；本轮没有新增浏览器流程变化。
