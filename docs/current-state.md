...
- 2026-07-05 Omni 模型下载：已通过 Mac 端 `127.0.0.1:7892` 代理建立 SSH reverse tunnel，服务器使用 `http://127.0.0.1:17892` 下载 `Qwen/Qwen2.5-Omni-7B-GPTQ-Int4` 到 `/home/aidev/models/Qwen2.5-Omni-7B-GPTQ-Int4`；模型仓库包含 21 个文件，4 个 safetensors 权重总量约 12.7GB。下载时需设置 `HF_HUB_DISABLE_XET=1`，否则容易卡在 Xet/LFS 链路。
...
6. 完成目标模型服务 Omni 部署：先在服务器本机执行 `scripts/server_prepare_qwen_omni.sh system-cuda` 补 `nvcc`，再执行 `PROXY_URL=http://127.0.0.1:17892 scripts/server_prepare_qwen_omni.sh gptq env check`；确认 `/load` 能加载 `Qwen/Qwen2.5-Omni-7B-GPTQ-Int4` 后，只对 15 秒以内短片段做 shadow-run，长视频先切 8-15 秒候选片段。
