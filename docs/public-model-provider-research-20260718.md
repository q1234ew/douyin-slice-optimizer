...
- 第二 Provider 对照：`kimi-k2.6`，仅在百炼 Shadow 基线稳定后接入。
...
Kimi K2.6 的 256K 上下文和原生图片/视频输入适合做长上下文或视频理解对照，但单价更高，且开放平台协议包含将输入输出用于服务优化的授权表述；在未获得更严格的企业数据条款前，不上传完整节目或未脱敏素材。
...
| --- | --- | --- | --- | --- |
| 阿里云百炼 | `qwen3.5-flash-2026-02-23` | 0–128K：输入 0.2、输出 2；128–256K：0.8/8；256K–1M：1.2/12。支持 Batch 半价和上下文缓存 | 中国内地北京地域；可创建子业务空间，限制模型、QPM/TPM，并给 API Key 配置模型范围和 IP 白名单；官方称调用数据不用于模型训练 | **首选，进入 `validate`** |
| 阿里云百炼 | `qwen3-vl-flash-2026-01-22` | 0–32K：0.15/1.5；32–128K：0.3/3；128–256K：0.6/6；Batch 半价 | OpenAI 兼容 Chat 支持 Qwen-VL；先上传 3 张代表帧和结构化摘要，不传完整视频 | **首个视觉 Shadow** |
...
1. 实现 `AliyunBailianProvider` 和 mock HTTP contract 测试，不配置真实密钥。
...
6. 百炼结果稳定后，才用同一冻结集接 `kimi-k2.6` 做第二 Provider 对照。
